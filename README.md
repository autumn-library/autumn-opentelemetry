...
Два обязательных параметра для запуска:

| Параметр | Описание |
|----------|----------|
| `otel.enabled` | Включает инструментирование аннотациями (`&Наблюдаемый`, `&Замеряемый`, `&Подсчитываемый`). По умолчанию `false` — SDK не инициализируется и экспортёры не запускаются. |
...
Чтобы отключить SDK полностью: `"otel": { "sdk": { "disabled": true } }`.
