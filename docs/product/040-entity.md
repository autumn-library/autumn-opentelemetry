...
На конструкторе аннотация покрывает методы самого класса хранилища; унаследованные от `ХранилищеСущностей` методы вроде `Получить` и `Сохранить` она не трогает, поэтому дублей с операциями entity не возникает. На отдельном методе - покрывает только его.
