...
Размещение на **конструкторе** инструментирует все экспортные методы бина, включая унаследованные от родителя:
...
## Аннотации на унаследованных методах

Напильники читают методы и их аннотации с самого желудя, а не из его класса, поэтому
аннотация на методе родителя работает ровно так же, как на методе самого класса: задаёт
своё имя спана или метрики, вид спана и атрибуты параметров. Аннотация на конструкторе
для этого не нужна - унаследованный метод инструментируется и без неё.
...
Так же инструментируются методы, добавленные желудю декоратором или построителем
наследника: в определении желудя их нет, а у объекта они есть.
